...
export const theme = (Component) => {
    return props => (
        <Consumer>
            {value => <Component {...props} theme={value["theme"]} changeTheme={value["changeTheme"]} />}
        </Consumer>
    );
}
...
export const getContextPropsFromState = (changeTheme) => {
    return createSelector(
        ...
        if (isNilOrEmpty(_defaultTheme)) throw new Error("valid defaultTheme are required");
        if (isNilOrEmpty(_activeTheme)) throw new Error("valid activeTheme are required");
